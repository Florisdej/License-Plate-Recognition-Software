...
def create_app(frontend_dir: Optional[str] = None) -> Flask:
    """Create and configure the Flask application.

    Args:
        frontend_dir: Path to the external frontend directory (contains
                      index.html + static/).  When provided and the directory
                      exists, the external SPA is served instead of the built-in
                      inline dashboard.  Pass None (default) to always use the
                      inline dashboard.

    The external frontend expects these endpoints to exist:
        GET  /api/status   — JSON status snapshot
        GET  /api/history  — JSON detection history
        POST /api/clear    — Clear history
        GET  /stream       — MJPEG live stream
    """
    import os

    _frontend_path: Optional[str] = None
    if frontend_dir:
        candidate = os.path.abspath(frontend_dir)
        if os.path.isfile(os.path.join(candidate, "index.html")):
            _frontend_path = candidate

    app = Flask(
        __name__,
        static_folder=os.path.join(_frontend_path, "static") if _frontend_path else None,
        static_url_path="/static",
    )

    # ----------------------------------------------------------------
    # Dashboard — serves external SPA or built-in inline HTML
    # ----------------------------------------------------------------
    @app.route("/")
    def dashboard():
        if _frontend_path:
